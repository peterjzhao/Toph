/** Read-only checks for the hosted mobile API, safe to run after a deployment. */
...
try {
  ...
  if (bootstrap.mode !== "shared" || !bootstrap.farm?.id || !bootstrap.accounts?.length || !bootstrap.fields?.length) {
    throw new Error("The mobile API did not return a farm, accounts, and fields.");
  }
  console.log(`PASS  Mobile API has ${bootstrap.accounts.length} account(s) and ${bootstrap.fields.length} field(s).`);
