...
export function FieldMap({ field, fields, imageUrl, zoom = 1, onSelect }: {
  ...
  const image = <img className={styles.image} src={source} alt={`Satellite map of ${field.name}`} draggable={false} width={!custom && region ? region.sourceSize.width : undefined} height={!custom && region ? region.sourceSize.height : undefined} />;
