...
export function MapPage() {
  ...
  return <div className={s.page}><PageHeader title="Map" description="Explore your fields and the work recorded on them"><Badge>{data.filterOptions.fields.length} fields</Badge></PageHeader>
    ...
    </div>{expanded && <Modal wide title={`${field?.name} · Satellite preview`} onClose={() => setExpanded(false)}>{renderMap()}<div className={s.mapCaption}>{zoomControls}<span className={s.workspaceNote}>Click a field to view its activity.</span></div></Modal>}
  </div>;
}
...
const helpArticles = [
  ...
  { title: "Does switching users change access permissions?", body: "Accounts share the same farm workspace and access. Switching chooses the current profile without a password. Signing out clears this browser’s selected account.", href: "/switch-user", label: "Switch account" },
];
...
export function AccountsPage({ login = false }: { login?: boolean }) {
  ...
  const cards = <><input type="search" aria-label="Find an account" className={s.searchInput} style={{ maxWidth: 370, marginBottom: 22 }} placeholder="Find a profile" value={search} onChange={event => setSearch(event.target.value)}/><div className={s.profiles}>{profiles.filter(person => `${person.name} ${person.role}`.toLowerCase().includes(search.toLowerCase())).map(person => <button className={s.profileCard} key={person.id} onClick={() => { switchUser(person.id); router.push("/"); }}><span className={s.initials}>{person.name.split(" ").map(word => word[0]).join("").slice(0,2)}</span><div><strong>{person.name}</strong><p>{person.role} · {workspace.settings.farmName}</p></div>{!login && session?.id === person.id ? <Badge tone="green">Current profile</Badge> : <span className={s.textButton}>{login ? "Enter workspace" : "Switch to profile"}<ArrowUpRight size={14}/></span>}</button>)}</div>{!profiles.some(person => `${person.name} ${person.role}`.toLowerCase().includes(search.toLowerCase())) && <EmptyState title="No matching profiles" description="Try another name or role."/>}</>;
  if (login) return <div className={s.login}><section className={s.loginCard}><Badge tone="green">Toph · Farm workspace</Badge><h1>Welcome to {workspace.settings.farmName}</h1><p>You’re signed out. Choose an account to continue.<br/>Profiles share the same farm data. No password is required.</p>{cards}</section></div>;
  return <div className={s.page}><PageHeader title="Switch User" description="Choose a profile to explore the farm workspace"/><p className={s.workspaceNote}>Accounts share the farm’s data and access. No password is required.</p>{cards}</div>;
}
