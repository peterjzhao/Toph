...
export async function POST(request: Request, context: { params: Promise<{ logId: string }> }) {
  return handleRoute(async () => {
    ...
    const rows = await ctx.sql`update toph.work_logs set is_new = false, reviewed_by = coalesce(reviewed_by, ${ctx.account.id}::uuid), reviewed_at = coalesce(reviewed_at, now()), updated_at = case when is_new then now() else updated_at end where farm_id = ${ctx.farmId} and id = ${id} returning id, is_new`;
