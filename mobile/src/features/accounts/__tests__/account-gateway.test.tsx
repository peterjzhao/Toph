...
jest.mock("@/lib/api/mobile-client", () => ({
  ...
  createMobileClient: () => Object.fromEntries(["session", "accounts", "login", "join", "logout"].map(key => [key, (...args: unknown[]) => mockApi[key](...args)])),
}));
...
const mockApi: Record<string, jest.Mock> = Object.fromEntries(["session", "accounts", "login", "join", "logout"].map(key => [key, jest.fn()]));
...
test("requires explicit login and saves a verified native session before opening the workspace", async () => {
  ...
  await fireEvent.changeText(screen.getByLabelText("Password"), "isaac");
  await fireEvent.press(screen.getByRole("button", { name: "Log in" }));
  await screen.findByText("Isaac Wang recording at Bays Ranch");
  expect(mockApi.login).toHaveBeenCalledWith("Isaac Wang", "isaac");
...
test("join uses name, a confirmed new password and farm code and preserves inputs after an invalid code", async () => {
  ...
  await fireEvent.changeText(screen.getByLabelText("Confirm password"), "long enough pass");
  ...
  expect(mockApi.join).not.toHaveBeenCalled();
  await fireEvent.changeText(screen.getByLabelText("Confirm password"), "long enough password");
  await fireEvent.press(screen.getByRole("button", { name: "Create account & join farm" }));
  await screen.findByText("This farm code is not valid.");
  expect(mockApi.join).toHaveBeenCalledWith("New Worker", "long enough password", "ABC123");
...
test("an admin identity never mounts the recording workspace", async () => {
  ...
  await fireEvent.changeText(screen.getByLabelText("Password"), "farm");
  await fireEvent.press(screen.getByRole("button", { name: "Log in" }));
